...
async function getTagPosts(tagId: string) {
  ...
  // Filter by tag on the client side since we need to join through content_tags
  return data || [];
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    title: `#${tag.name} - ACM Media Platform`,
  ...
  return (
    <>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <p className="text-lg text-gray-600">Articles tagged with {tag.name}</p>
      </div>
